...
public class StringMergeSorter
{
	 ...
	 */
   public static void sort(String[] a, boolean debug)
   {
 ...
 */
   public static void merge(int from, int mid, int to, String[] a, boolean debug)
   {
      if (debug)
      ...
      String[] merge = new String[to - from + 1];
      int l = 0; // from the first
      int r = 0; // from the second
      int i = 0; // open position
      ...
      {    	  
    	  ...
    	  merge[i++] = (a[from + l].compareTo(a[midSt + r]) < 0) ? a[from + l++] : a[midSt + r++];
      }
