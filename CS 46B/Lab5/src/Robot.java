...
public class Robot
{
   ...
   public void escape() // escape maze
   {
	   if (this.atExit()) // robot already starting at exit
	   {
		   System.out.println("Using first escape method."); // return
		   System.out.println("Robot at exit."); // return 
	   }
	   else
	   {
		   if (this.canMove())
   ...
   public boolean escape2() // checks for already visited locations
   {
      if (atExit()) return true;
      // this loop makes the robot check all possible direction
      for (int i = 1; i <= 4; i++)
      {
         turnRight();
         if (canMove())
         {
            ...
            if (cloned.escape2())
            {
               visited.add(getPosition()); // already visited that location
               return true;
            }
         }
      }
